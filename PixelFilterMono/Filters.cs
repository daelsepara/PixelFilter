...
public static class Filters
{
    ...
    {
        if (Common.OSTest.IsWindows() ? File.Exists(String.Format("./libpixel++{0}.dll", library)) : (Common.OSTest.IsRunningOnMac() ? File.Exists(String.Format("./libpixel++{0}.dylib", library)) : File.Exists(String.Format("./libpixel++{0}.so", library))))
        {
